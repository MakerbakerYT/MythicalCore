...
class PresentGiftChooser {
    ...
    public function __construct() {
        $this->rewards = [
            new Reward("Lucky Block", Item::get(Item::NAME_TAG, 0, 1), function(MythicalPlayer $player): void {
                $item = (new LuckyBlock(mt_rand(75, 100)))->getItemForm();
                $player->getInventory()->addItem($item);
            }, 1000),
            new Reward("Sacred Stone", Item::get(Item::NETHER_QUARTZ, 0, 1), function(MythicalPlayer $player): void {
                $item = (new SacredStone())->getItemForm();
                $player->getInventory()->addItem($item);
            }, 100),
            new Reward("Holy Box", Item::get(Item::CHEST, 0, 1), function(MythicalPlayer $player): void {
                $kits = Mythical::getInstance()->getKitManager()->getSacredKits();
            ...
            new Reward("$1,000,000", Item::get(Item::PAPER, 0, 1), function(MythicalPlayer $player): void {
                $player->addToBalance(1000000);
            }, 100),
            new Reward("$10,000,000", Item::get(Item::PAPER, 0, 1), function(MythicalPlayer $player): void {
                $player->addToBalance(10000000);
            }, 10),
            new Reward("God Rank", Item::get(Item::PAPER, 0, 1), function(MythicalPlayer $player): void {
                $player->setRank($player->getCore()->getRankManager()->getRankByIdentifier(Rank::GOD));
            }, 1),
            new Reward("Warlord Rank", Item::get(Item::PAPER, 0, 1), function(MythicalPlayer $player): void {
                $player->setRank($player->getCore()->getRankManager()->getRankByIdentifier(Rank::WARLORD));
            }, 1),
            new Reward("Sell Wand", Item::get(Item::DIAMOND_HOE, 0, 1), function(MythicalPlayer $player): void {
