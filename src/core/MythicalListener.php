...
class MythicalListener implements Listener
{
    ...
    {
        $this->core = $core;
        $this->messages[] = Mythical::SERVER_NAME;
        $this->messages = array_merge($this->messages, Mythical::MESSAGES);
    }
    ...
    {
        if (!$e->isCancelled() and $e->getPlayer()->getLevel()->getFolderName() == Mythical::WORLD and !$e->getPlayer()->isOp()) {
            $e->setCancelled();
        }
    }
    ...
    public function onPlayerJoin(PlayerJoinEvent $event): void
    {
        ...
        $max = $maxPlayers - Mythical::EXTRA_SLOTS;
        ...
        /** @var MythicalPlayer $onlinePlayer */
        ...
        $this->core->getScheduler()->scheduleDelayedTask extends Task {
        {
             ...
             */
            public function __construct(MythicalPlayer $player)
            {
                $this->player = $player;
            }
            ...
            public function onRun(int $currentTick)
            {
                ...
                $pk->data = 0;
                $this->player->sendDataPacket($pk);
                $this->player->addTitle("  ", "§l§6Mythical§eNetwork§r\n§b" . Mythical::GAMEMODE . "\n\n\n\n\n\n\n", 5, 20, 5);
...
//				}
    ...
    {
        $event->setPlayerClass(MythicalPlayer::class);
    }
    ...
    public function onPlayerCommandPreprocess(PlayerCommandPreprocessEvent $event): void
    {
        ...
        $event->setCancelled();
        $player->sendMessage(Translation::getMessage("[!] Warning restarting Soon [!]"));
    }
    ...
    public function onPlayerMove(PlayerMoveEvent $event)
    {
        ...
        if ($x >= Mythical::BORDER) {
            $player->teleport(new Vector3($x - 1, $y, Mythical::BORDER - 1));
            $player->sendMessage($message);
        }
        if ($z >= Mythical::BORDER) {
            $player->teleport(new Vector3($x, $y, Mythical::BORDER - 1));
            $player->sendMessage($message);
        }
        if ($x >= Mythical::BORDER and abs($z) >= Cryptic::BORDER) {
            $player->teleport(new Vector3(Cryptic::BORDER - 1, $y, Mythical::BORDER - 1));
            $player->sendMessage($message);
        }
    }
    ...
    public function onQueryRegenerate(QueryRegenerateEvent $event): void
    {
        ...
        $maxPlayers = $this->core->getServer()->getMaxPlayers();
        $maxSlots = $maxPlayers - Mythical::EXTRA_SLOTS;
...
//                        $max = 10;
//                        $cfg = Mythical::getInstance()->getHead();
        ...
        if ($player instanceof MythicalPlayer) {
            if ($player->isInStaffMode()) {
                ...
                switch ($item->getId()) {
                    case Item::CONCRETE:
                        if ($item->getDamage() === 5) {
                            $player->setChatMode(MythicalPlayer::PUBLIC);
                            ...
                            $player->setChatMode(MythicalPlayer::STAFF);
        ...
        if ($player instanceof MythicalPlayer) {
            if (substr($event->getMessage(), 0, 1) === "/") {
                ...
                if (strtolower($command) === "tp" or strtolower($command) === "teleport") {
                    if ($player->isInStaffMode()) {
                        $player->sendMessage("§l§8(§a!§8)§r §7Im Sorry But You Cant Use Staff Mode");
                        $event->setCancelled();
                        return;
                    }
                }
