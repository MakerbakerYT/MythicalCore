...
class Naddy {
    ...
    public function getLastTalked(): ?CommandSender
    {
        ...
        if (!$this->lastTalked instanceof MythicalPlayer) {
            return null;
        }
     ...
     * @param Cryptic $core
     */
    public function load(Mythical $core): void
    {
        $this->core = $core;
        ...
        $this->rewards->setInventoryCloseListener(function (Player $player, BaseFakeInventory $inventory): void {
            $xuid = $player->getXuid();
            $items = Mythical::encodeInventory($inventory);
        ...
        $stmt->close();
        if ($items !== null) {
            $items = Mythical::decodeInventory($items);
            foreach ($items as $item) {
                $this->rewards->getInventory()->addItem($item);
            }
        }
        if ($inbox !== null) {
            $items = Mythical::decodeInventory($inbox);
            $i = 0;
            foreach ($items as $item) {
     ...
     * @return Cryptic
     */
    public function getCore(): Mythical
    {
        return $this->core;
    }
    ...
    public function addToInbox(Item $item): void
    {
        ...
        $xuid = $this->getXuid();
        $items = Mythical::encodeInventory($this->inbox->getInventory());
        $stmt = $this->core->getMySQLProvider()->getDatabase()->prepare("UPDATE inboxes SET items = ? WHERE xuid = ?");
        $stmt->bind_param("ss", $items, $xuid);
        $stmt->execute();
        $stmt->close();
     ...
     * @return bool
     */
    public function isRequestingTeleport(MythicalPlayer $player): bool
    {
     ...
     * @param CrypticPlayer $player
     */
    public function addTeleportRequest(MythicalPlayer $player): void
    {
     ...
     * @param CrypticPlayer $player
     */
    public function removeTeleportRequest(MythicalPlayer $player): void
    {
     ...
     * @return bool
     */
    public function isRequestingTrade(MythicalPlayer $player): bool
    {
     ...
     * @param CrypticPlayer $player
     */
    public function addTradeRequest(MythicalPlayer $player): void
    {
     ...
     * @param CrypticPlayer $player
     */
    public function removeTradeRequest(MythicalPlayer $player): void
    {
    ...
    public function togglePVPHUD(): void
    {
        ...
        if ($this->pvpHud === false) {
            ...
            $this->scoreboard->setScoreLine(9, TextFormat::RESET . TextFormat::AQUA . "vote:Coming Soon");
