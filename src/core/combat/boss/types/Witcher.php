...
class Witcher extends Boss {
    ...
    public function __construct(Level $level, CompoundTag $nbt) {
        $path = Mythical::getInstance()->getDataFolder() . "skins" . DIRECTORY_SEPARATOR . "witcher.png";
    ...
    public function onDeath(): void {
        ...
        $crate = Mythical::getInstance()->getCrateManager()->getCrate($rand);
        if($p instanceof MythicalPlayer){
        ...
        foreach($this->damages as $player => $damage) {
            ...
            if($p instanceof MythicalPlayer){
                $crate = Mythical::getInstance()->getCrateManager()->getCrate("Rare");
