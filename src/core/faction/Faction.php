...
class Faction {
    ...
    public function getOnlineMembers(): array {
        ...
        foreach($this->members as $member) {
            $player = Mythical::getInstance()->getServer()->getPlayer($member);
            if($player !== null) {
                $members[] = $player;
            }
        }
    ...
    public function isInFaction($player): bool {
        $player = $player instanceof MythicalPlayer ? $player->getName() : $player;
     ...
     */
    public function demote(MythicalPlayer $player): void {
     ...
     * @param CrypticPlayer $player
     */
    public function promote(MythicalPlayer $player): void {
        $player->setFactionRole($player->getFactionRole() + 1);
    }
     ...
     * @param CrypticPlayer $player
     */
    public function addMember(MythicalPlayer $player): void {
        $this->members[] = $player->getName();
        $player->setFactionRole(self::RECRUIT);
        $player->setFaction($this);
        $members = implode(",", $this->members);
        $stmt = Mythical::getInstance()->getMySQLProvider()->getDatabase()->prepare("UPDATE factions SET members = ? WHERE name = ?");
    ...
    public function removeMember($player): void {
        $name = $player instanceof MythicalPlayer ? $player->getName() : $player;
     ...
     * @return bool
     */
    public function isInvited(MythicalPlayer $player): bool {
     ...
     */
    public function addInvite(MythicalPlayer $player): void {
     ...
     * @param MythicalPlayer $player
     */
    public function removeInvite(MythicalPlayer $player): void {
    ...
    public function addAlly(Faction $faction): void {
        ...
        $stmt = Mythical::getInstance()->getMySQLProvider()->getDatabase()->prepare("UPDATE factions SET allies = ? WHERE name = ?");
    ...
    public function addMoney(int $amount): void {
        $this->balance += $amount;
        $stmt = Mythical::getInstance()->getMySQLProvider()->getDatabase()->prepare("UPDATE factions SET balance = balance + ? WHERE name = ?");
    ...
    public function addStrength(int $amount): void {
        ...
        $stmt = Mythical::getInstance()->getMySQLProvider()->getDatabase()->prepare("UPDATE factions SET strength = strength + ? WHERE name = ?");
        $stmt->bind_param("is", $amount, $this->name);
        $stmt->execute();
        $stmt->close();
    }
    ...
    public function setHome(?Position $position = null): void {
        ...
        $stmt = Mythical::getInstance()->getMySQLProvider()->getDatabase()->prepare("UPDATE factions SET x = ?, y = ?, z = ? WHERE name = ?");
