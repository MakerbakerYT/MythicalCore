...
class TagCrate extends Crate {
    ...
    public function __construct(Position $position) {
        $tag = Mythical::getInstance()->getTagManager();
        parent::__construct(self::TAG, $position, [
        new Reward("Cryptic4Lyfe Tag", $tag->getTagNote("Cryptic4Lyfe"), function(MythicalPlayer $player)use($tag): void {
                $player->getInventory()->addItem($tag->getTagNote("Cryptic4Lyfe"));
            }, 45),
        new Reward("Godly Tag", $tag->getTagNote("Godly"), function(MythicalPlayer $player)use($tag): void {
                $player->getInventory()->addItem($tag->getTagNote("Godly"));
            }, 45),
        new Reward("OG Tag", $tag->getTagNote("OG"), function(MythicalPlayer $player)use($tag): void {
                $player->getInventory()->addItem($tag->getTagNote("OG"));
            }, 45),
        new Reward("Toxic Tag", $tag->getTagNote("Toxic"), function(MythicalPlayer $player)use($tag): void {
                $player->getInventory()->addItem($tag->getTagNote("Toxic"));
            }, 45),
        new Reward("Rusty Tag", $tag->getTagNote("Rusty"), function(MythicalPlayer $player)use($tag): void {
                $player->getInventory()->addItem($tag->getTagNote("Rusty"));
            }, 45),
        new Reward("Sweat Tag", $tag->getTagNote("Sweat"), function(MythicalPlayer $player)use($tag): void {
                $player->getInventory()->addItem($tag->getTagNote("Sweat"));
            }, 45),
        new Reward("Tryhard Tag", $tag->getTagNote("Tryhard"), function(MythicalPlayer $player)use($tag): void {
                $player->getInventory()->addItem($tag->getTagNote("Tryhard"));
            }, 45),
        new Reward("EGIRL Tag", $tag->getTagNote("EGIRL"), function(MythicalPlayer $player)use($tag): void {
                $player->getInventory()->addItem($tag->getTagNote("EGIRL"));
            }, 45),
        new Reward("Bot Tag", $tag->getTagNote("Bot"), function(MythicalPlayer $player)use($tag): void {
                $player->getInventory()->addItem($tag->getTagNote("Bot"));
            }, 45),
        new Reward("Bot Tag", $tag->getTagNote("Bot"), function(MythicalPlayer $player)use($tag): void {
                $player->getInventory()->addItem($tag->getTagNote("Bot"));
            }, 45),
        new Reward("Minemen Tag", $tag->getTagNote("Minemen"), function(MythicalPlayer $player)use($tag): void {
                $player->getInventory()->addItem($tag->getTagNote("Minemen"));
            }, 45),
        new Reward("EZPZ Tag", $tag->getTagNote("EZPZ"), function(MythicalPlayer $player)use($tag): void {
                $player->getInventory()->addItem($tag->getTagNote("EZPZ"));
            }, 45),
        new Reward("HypeBeast Tag", $tag->getTagNote("HypeBeast"), function(MythicalPlayer $player)use($tag): void {
